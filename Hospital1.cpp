...
void Lista::insertarEnOrden(int dia, int mes, int anio, int hora, int minutos, string nombreMedico, int numConsultorio)
{
    ...
    else
    {
        ...
        if (anterior == nullptr)
        {
            // El nuevo nodo va al principio
            nuevo->siguiente = raiz;
            raiz = nuevo;
        }
        else
        {
            // El nuevo nodo va en medio o al final
...
void Cita::modificarCita(int indiceCita){
    Cita cita;
    Paciente paciente;

    Lista* listaCitas = paciente.getCitas();
   if(listaCitas && indiceCita >= 0 && indiceCita < indice) {
        // Acceder a la cita específica y modificarla
        Lista& citaModificar = listaCitas[indiceCita]; 
    ...
    do{
        ...
        cout << "El indice de lista no es valido"<<endl;
    }
}
...
int main()
{
    ...
    NodoAVL *Pencontrar, *anterior, *actual, *siguiente;
    bool encontrado;
    AVLTree arbol;
    AVLTree avl;
    int numCitas;
        ...
        case 2:
            ...
            else
            {
                ...
                if (!(Pencontrar == nullptr))
                {
                    ...
                    Lista* listaCitas = nuevoPaciente.getCitas();
                    nuevoPaciente.getCitas().
                    if(listaCitas && numCitas >= 0 && numCitas < nuevoPaciente.obtenerNumCitas()) {
                        // Acceder a la cita específica y modificarla
                         Lista& citaModificar = listaCitas[numCitas]; 
                    }
                    Pencontrar->paciente.getCitas()
